...
void AttribuerCommandeLivreur::onCommandesReceived(QNetworkReply *reply) {
    if (reply->error() == QNetworkReply::NoError) {
        ...
        for (const QJsonValue &value : commandesArray) {
            ...
            // Désactiver la commande si son statut est "en cours"
            if (statutCommande == "en cours") {
                int index = ui->comboBoxCommandes->count() - 1;  // Dernier élément ajouté
                ui->comboBoxCommandes->setItemData(index, false, Qt::ItemIsEnabled);  // Désactiver la commande
            }
        }
